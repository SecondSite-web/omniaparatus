...
$sql = "CREATE TABLE IF NOT EXISTS `phpauth_users` (
  ...
  KEY `email` (`email`)
      ...
      if ($conn->query($sql) === TRUE) {
          echo '<div class="alert alert-success">' . "Table phpauth_users created successfully". '</div>';
      } else {
...
if ($conn->query($sql) === TRUE) {
    echo '<div class="alert alert-success">'."Table phpauth_config created successfully". '</div>';
} else {
    echo '<div class="alert alert-danger">'."Error creating table: " . $conn->error. '</div>';
}
unset($sql);
...
if ($conn->query($sql) === TRUE) {
    echo '<div class="alert alert-success">'."Table twig_settings created successfully". '</div>';
} else {
    echo '<div class="alert alert-danger">'."Error creating table: " . $conn->error. '</div>';
}
unset($sql);

$sql = "CREATE TABLE IF NOT EXISTS `rental_settings` (
`setting` varchar(100) NOT NULL,
`value` varchar(100) DEFAULT NULL,
...
if ($conn->query($sql) === TRUE) {
  echo '<div class="alert alert-success">'."Table rental_settings created successfully". '</div>';
} else {
  echo '<div class="alert alert-danger">'."Error creating table: " . $conn->error. '</div>';
}
unset($sql);
...
if ($conn->query($sql) === TRUE) {
  echo '<div class="alert alert-success">'."Twig Template settings successfully stored to DB". '</div>';
} else {
    echo '<div class="alert alert-danger">'."Error inputting settings to table: " . $conn->error. '</div>';
}
unset($sql);
...
if ($conn->query($sql) === TRUE) {
    echo '<div class="alert alert-success">'."PHPAuth settings successfully stored to DB". '</div>>';
} else {
    echo '<div class="alert alert-danger">'."Error saving settings to table: " . $conn->error. '</div>';
}
unset($sql);?>
          <hr />
      ...
      <div class="row text-center pb-2 mb-2">
          <div class="col-12 align-content-center text-center">
              <a href="email-banned.php" class="btn btn-primary btn-lg" target="_self">Email Banned List</a>
              <br>
          </div>
      </div>
    </div>
